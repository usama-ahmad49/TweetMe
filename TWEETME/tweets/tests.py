...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(username='abc-2', password='password')
        Tweet.objects.create(content='myFirsttweet', user=self.user)
        Tweet.objects.create(content='myFirsttweet', user=self.user)
        Tweet.objects.create(content='myFirsttweet', user=self.user2)
    ...
    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content='mySecondtweet', user=self.user)
        self.assertEqual(tweet_obj.id, 1)
        self.assertEqual(tweet_obj.obj.user, self.user)
    ...
    def test_action_unlike(self):
        client = self.get_client()
        response = client.get("/api/tweets/action/", {"id": 2, "action": "like"})
        like_count = response.json().get("likes")
        self.assertEqual(response.status_code, 200)
        response = client.get("/api/tweets/action/", {"id": 2, "action": "unlike"})
        self.assertEqual(response.status_code, 200)
        like_count = response.json().get("likes")
    ...
    def test_tweet_delete_api_view(self):
        ...
        response_incorrect = client.delete("/api/tweets/3/delete/")
